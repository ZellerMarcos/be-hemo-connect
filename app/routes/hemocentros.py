...
def read_hemocentros(db: Session = Depends(get_db)):
    return list_hemocentros(db)


@router.get("/{hemocentro_id}", response_model=HemocentroResponse)
def read_hemocentro(hemocentro_id: int, db: Session = Depends(get_db)):
...
def create_hemocentro_route(
    data: HemocentroCreate, db: Session = Depends(get_db)
):
...
def delete_hemocentro_route(hemocentro_id: int, db: Session = Depends(get_db)):
    hemocentro = find_or_404(db, hemocentro_id)
